...
/////////////////////////////////////////////////////////////////////////////////////////
//// Regular CUDA kernel -w 5 MAC + IP address swap
/////////////////////////////////////////////////////////////////////////////////////////
__global__ void kernel_mac_ip_update(struct rte_gpu_comm_list *comm_list, uint64_t wtime_n)
{
	int idx = blockIdx.x * blockDim.x + threadIdx.x;
	uint16_t temp;
	unsigned long long pkt_start;
	...
	if (idx < comm_list->num_pkts && comm_list->pkt_list[idx].addr != 0) {
		...
		uint16_t *src_addr = (uint16_t *) (&eth->src_addr);
		uint16_t *dst_addr = (uint16_t *) (&eth->dst_addr);
		struct rte_ipv4_hdr *ip_hdr = (struct rte_ipv4_hdr *)((uint8_t *) eth + sizeof(struct rte_ether_hdr));
		rte_be32_t tmp_ip_addr = ip_hdr->src_addr;
							...
							int cuda_blocks, int cuda_threads, cudaStream_t stream)
{
	...
	CUDA_CHECK(cudaGetLastError());
	kernel_mac_ip_update <<< cuda_blocks, cuda_threads, 0, stream >>> (comm_list, wtime_n);
	CUDA_CHECK(cudaGetLastError());
}
